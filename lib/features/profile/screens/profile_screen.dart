...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadNotifPreference() async {
    ...
    if (mounted) setState(() => _notifikasi = enabled);
    if (enabled) {
      await NotificationService().scheduleAllBookingReminders();
    }
  }

  // ── Toggle notifikasi ───────────────────────────────────────────────────
  Future<void> _toggleNotifikasi(bool val) async {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Row(children: [
              ...
              SizedBox(width: 10),
              Text('Notifikasi booking diaktifkan'),
            ]),
          ...
          SnackBar(
            content: const Row(children: [
              Icon(Icons.notifications_off_rounded,
                  color: Colors.white, size: 18),
              SizedBox(width: 10),
              Text('Notifikasi booking dimatikan'),
            ]),
      ...
      if (doc.exists && mounted) {
        ...
        if (mounted) {
          setState(() {
            ...
            _totalUlasan  = ulasanQuery.docs.length;
            _loadingUser  = false;
          });
      ...
      loadingBuilder: (_, child, progress) {
        ...
        return Center(
          child: CircularProgressIndicator(
              strokeWidth: 2, color: Colors.white.withOpacity(0.8)),
        );
      },
                      ...
                      children: [
                        ...
                        _buildMenuCard([
                          _toggleItem(
                            ...
                            'Notifikasi',
                            _notifikasi,
                            _toggleNotifikasi, // ← pakai method baru
                          ),
                        ]),
                        const SizedBox(height: 24),
  ...
  Widget _buildStatsCard() {
    return Container(
      ...
      child: Row(
        children: [
          _statItem(
              _loadingUser ? '-' : _totalBooking.toString(), 'TOTAL BOOKING'),
          Container(width: 1, height: 36, color: Colors.grey.shade200),
          _statItem(_loadingUser ? '-' : _totalUlasan.toString(), 'ULASAN'),
        ],
      ),
    );
  }
