            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  OutlinedButton.icon(
                    onPressed: () =>
                        _snack('Fitur ekspor segera hadir'),
                  ...
                  ElevatedButton.icon(
                    onPressed: () =>
                        _snack('Filter lanjutan segera hadir'),
                    ...
                    if (pageItems.isEmpty)
                      const Padding(
                        ...
                        child: Center(
                          ...
                          SizedBox(height: 4),
                          Text(
                              'Tambah jadwal baru dengan tombol di atas',
                              style: TextStyle(
                                  fontSize: 13,
                ...
                Row(children: [
                  Text(
                      'Menampilkan ${(_currentPage - 1) * _perPage + 1}-${(_currentPage * _perPage).clamp(0, all.length)} dari ${all.length} entri',
                      style: const TextStyle(
                          fontSize: 12.5,
                          color: Color(0xFF9E9E9E))),
                  const Spacer(),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Text(value,
                style: TextStyle(
                    ...
                    fontWeight: FontWeight.w800,
                    color:
                        isHighlighted ? Colors.white : _textDark)),
          ]),
        ]),
      );
...
// ══════════════════════════════════════════════════════════════════════════════
// DIALOG TAMBAH JADWAL — dengan Generate Slot Otomatis
// ══════════════════════════════════════════════════════════════════════════════
...
class _TambahJadwalDialog extends StatefulWidget {
  ...
  @override
  State<_TambahJadwalDialog> createState() =>
      _TambahJadwalDialogState();
}

class _TambahJadwalDialogState
    extends State<_TambahJadwalDialog> {
  static const _primary   = Color(0xFF1565C0);
  static const _borderCol = Color(0xFFDDE3EE);

  // ── Mode: single atau bulk ────────────────────────────────
  bool _isBulkMode = true; // default bulk (generate otomatis)
  ...
  // ── Bulk mode ─────────────────────────────────────────────
  String _jamBuka  = '06:00'; // jam mulai operasional
  String _jamTutup = '21:00'; // jam tutup operasional
  ...
  List<Map<String, String>> get _previewSlots {
    ...
    for (int h = startH; h < endH; h++) {
      final mulai   = '${h.toString().padLeft(2, '0')}:00';
      final selesai = '${(h + 1).toString().padLeft(2, '0')}:00';
      result.add({'mulai': mulai, 'selesai': selesai});
    }
    return result;
  }
