...
import 'package:intl/intl.dart'; // Digunakan untuk format mata uang (akuntansi)
...
class FieldItem {
  ...
  final DateTime? createdAt; // Ditambahkan untuk menghitung statis bulan ini
...
class ActivityItem {
  ...
  factory ActivityItem.fromFirestore(DocumentSnapshot doc) {
    ...
    // FIX: Pewarnaan bullet berdasarkan tipe lapangan dengan prioritas lebih baik
    String fieldType = (data['field_type'] ?? '').toString().toLowerCase();
    String activityType = (data['activity_type'] ?? '').toString().toLowerCase();
    Color color = Colors.grey;

    // Prioritaskan field_type untuk warna bullet jika tersedia
    if (fieldType.isNotEmpty) {
      if (fieldType.contains('futsal')) {
        color = Colors.blue;
      } else if (fieldType.contains('bulutangkis') || fieldType.contains('badminton')) {
        color = Colors.orange;
      } else if (fieldType.contains('basket')) {
        color = Colors.purple;
      } else if (fieldType.contains('padel')) {
        color = Colors.green;
      } else if (fieldType.contains('tenis') || fieldType.contains('tennis')) {
        color = Colors.teal;
      } else if (fieldType.contains('voli') || fieldType.contains('volleyball')) {
        color = Colors.red;
      } else {
        // Fallback ke activity_type jika field_type tidak dikenali
      ...
      // Jika field_type kosong, gunakan activity_type
      if (activityType == 'update') {
        color = Colors.orange;
      } else if (activityType == 'delete') {
        color = Colors.red;
...
class _KelolaLapanganScreenState extends State<KelolaLapanganScreen> {
  ...
  // Format Harga ke Akuntansi (cth: Rp 150.000,00)
  ...
  // FIX: Navigasi ke AddField dengan pushReplacement-safe pattern
  // Menggunakan mounted check dan setState setelah kembali untuk memastikan
  // sidebar dan scaffold tetap utuh
  void _navigateToAddField() async {
    ...
    // FIX: Pastikan widget masih mounted sebelum melakukan setState / operasi lain
    if (!mounted) return;
    // Asumsi: AddFieldScreen me-return field_type / status sukses
    if (result != null && result is Map) {
      await _logActivity(
        'create',
        'Lapangan baru ditambahkan',
        'Admin menambah lapangan ke inventaris',
        result['type'] ?? '',
      );
    }
    // FIX: Paksa rebuild untuk memastikan sidebar dan UI kembali konsisten
    if (mounted) setState(() {});
  }
  ...
  Future<void> _logActivity(
    ...
    await FirebaseFirestore.instance.collection('aktivitas_lapangan').add({
      ...
      'title': title,
      'subtitle': subtitle,
      'field_type': fieldType, // Disimpan untuk memberi warna pada bullet aktivitas
  ...
  void _showActionMenu(BuildContext context, FieldItem field, Offset offset) {
    showMenu<String>(
      ...
      switch (value) {
        case 'edit':
          // FIX: Pastikan context masih mounted sebelum navigasi
          ...
          // FIX: Pastikan mounted setelah kembali dari navigasi
          if (!context.mounted) return;
          // Mencegah log jika admin hanya buka edit dan membatalkan/back tanpa men-save (result harus == true)
          if (result == true) {
            await _logActivity(
              'update',
          ...
          // FIX: Trigger setState untuk memastikan UI konsisten tanpa perlu refresh
          if (mounted) setState(() {});
          break;
        case 'toggle':
          ...
          content: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  FilterChip(
                    label: Text("Semua", style: GoogleFonts.inter(fontSize: 12)),
            ...
            ElevatedButton(
              onPressed: () {
                setState(() {
                  ...
                  _currentPage = 0; // Reset paginasi saat filter di-apply
                });
                Navigator.pop(ctx);
              },
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, fieldSnapshot) {
                ...
                // Logika Paginasi (Maksimal 4 Lapangan)
                int totalPages =
                    (filteredFields.length / _itemsPerPage).ceil();
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          _buildSummaryCard(
                              "Tersedia", tersedia.toString(), Icons.check_circle_outline, null),
                          const SizedBox(width: 16),
                          _buildSummaryCard(
                              "Kategori", kategori.toString(), Icons.layers_outlined, null),
                        ],
                      ),
                      const SizedBox(height: 16),
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                OutlinedButton.icon(
                                  onPressed: () =>
                                      _showFilterDialog(context, availableTypes),
                            ...
                            filteredFields.isEmpty
                                ...
                                : Column(
                                    children: [
                                      // FIX: Tambahkan Theme wrapper untuk menghilangkan
                                      // highlight warna biru pada heading row DataTable
                                      Theme(
                                        ...
                                        child: SizedBox(
                                          ...
                                          child: DataTable(
                                            ...
                                            horizontalMargin: 0,
                                            columnSpacing: 16,
                                            // FIX: Set headingRowColor eksplisit ke putih
                                            headingRowColor:
                                                WidgetStateProperty.all(
                                                    Colors.white),
                                      ...
                                      // FIX: KONTROL PAGINASI - Desain baru sesuai screenshot
                                      Row(
                                        mainAxisAlignment:
                                        ...
                                        children: [
                                          ...
                                          // FIX: Pagination button sesuai design (kotak dengan border)
                        ...
                        builder: (context, actSnapshot) {
                          ...
                          List<ActivityItem> activities = actSnapshot.hasData
                              ? actSnapshot.data!.docs
                                  .map((doc) =>
                                      ActivityItem.fromFirestore(doc))
                                  .toList()
                              : [];
                                                  ...
                                                  children: [
                                                    Padding(
                                                      ...
                                                      child: Icon(
                                                        Icons.circle,
                                                        size: 8,
                                                        // FIX: Warna bullet variatif sesuai tipe lapangan
                                                        color: act.dotColor,
                                                      ),
                                                    ),
  ...
  // FIX: Widget pagination baru sesuai design (tombol kotak dengan border, nomor halaman)
  Widget _buildPaginationControls(int totalPages) {
    ...
    return Row(
      ...
      children: [
        ...
        // Nomor halaman (tampilkan maksimal 3 nomor halaman)
        ...List.generate(totalPages, (index) {
          // Jika banyak halaman, tampilkan hanya yang relevan
          if (totalPages > 5) {
            if (index != 0 &&
                index != totalPages - 1 &&
  ...
  // Tombol navigasi (< dan >) sesuai design - kotak putih dengan border abu
  ...
  // Tombol nomor halaman sesuai design - aktif biru, tidak aktif putih dengan border
  ...
  DataRow _buildDataRow(FieldItem field) {
    return DataRow(
      // FIX: Set warna row eksplisit ke putih agar tidak ada highlight biru
