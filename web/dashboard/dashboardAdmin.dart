...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _fetchStats() async {
    try {
      ...
      final todayQ = await _firestore.collection('bookings')
          .where('tanggal_booking', isGreaterThanOrEqualTo: Timestamp.fromDate(todayStart))
          .where('tanggal_booking', isLessThan: Timestamp.fromDate(todayEnd))
          .get();
      ...
      final bulanLatuQ = await _firestore.collection('bookings')
          .where('tanggal_booking', isGreaterThanOrEqualTo: Timestamp.fromDate(startLast))
          .where('tanggal_booking', isLessThan: Timestamp.fromDate(endLast))
          .where('status_pembayaran', isEqualTo: 'paid')
          .get();

      double ini  = 0;
      double lalu = 0;
      for (final d in bulanIniQ.docs)  ini  += double.tryParse(d['total_harga'].toString()) ?? 0;
      for (final d in bulanLatuQ.docs) lalu += double.tryParse(d['total_harga'].toString()) ?? 0;
  ...
  Future<void> _fetchBookingTerbaru() async {
    try {
      final q = await _firestore.collection('bookings')
          ...
          .get();
      if (mounted) setState(() {
        _bookingTerbaru = q.docs.map((d) => {'id': d.id, ...d.data()}).toList();
  ...
  Future<void> _fetchLapangan() async {
    try {
      ...
      if (mounted) setState(() {
        _lapanganList  = q.docs.map((d) => {'id': d.id, ...d.data()}).toList();
        _totalLapangan = _lapanganList.length;
        _lapanganAktif = _lapanganList.where((l) =>
            l['status'] == 'aktif' || l['status'] == null).length;
      });
    } catch (_) {}
  }

  // ── Helpers ───────────────────────────────────────────────────────────────
  String _rp(double v) => 'Rp ${NumberFormat('#,###', 'id_ID').format(v.toInt())}';
  ...
  double get _growth => _pendapatanBulanLalu == 0 ? 0
      : ((_pendapatanBulanIni - _pendapatanBulanLalu) / _pendapatanBulanLalu) * 100;

  double get _kapPct => _totalLapangan == 0 ? 0
              ...
              child: Row(
                children: [
                  ...
                  if (_expanded) ...[
                    ...
                    Icon(Icons.chevron_left_rounded, color: _muted, size: 20),
                  ],
                ],
              ),
            ),
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: active
                      ? Border(left: BorderSide(color: _blue, width: 3))
                      : null,
                ),
                child: Row(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.person_rounded, color: _blue, size: 20),
                ),
                if (_expanded) ...[
                  const SizedBox(width: 10),
                  Expanded(
          ...
          Stack(children: [
            Container(
              ...
              decoration: BoxDecoration(color: _bg,
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(color: _border)),
              child: Icon(Icons.notifications_outlined, size: 20, color: _muted),
            ),
        ...
        Wrap(
          ...
          children: [
            ...
            Container(
              ...
              child: Text(lbl,
                  style: _t(size: 11, weight: FontWeight.w700, color: lc)),
            ),
          ],
        ),
        const SizedBox(height: 12),
  ...
  Widget _sRow(String name, Color dot) => Padding(
    ...
    child: Row(children: [
      Expanded(child: Text(name, style: _t(size: 13), maxLines: 1, overflow: TextOverflow.ellipsis)),
  ...
  Widget _buildTableBooking() {
    final list = _searchQuery.isEmpty
        ...
        : _bookingTerbaru.where((b) {
            ...
            final l = (b['nama_lapangan'] ?? '').toString().toLowerCase();
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const Spacer(),
            TextButton(
              onPressed: _fetchBookingTerbaru,
        ...
        Padding(
          ...
          child: Row(children: [
            _th('PELANGGAN', 3), _th('LAYANAN', 2), _th('WAKTU', 2),
            _th('TOTAL', 2), _th('STATUS', 2), _th('AKSI', 1),
          ]),
        ),
        const SizedBox(height: 8),
      ...
      Padding(
        ...
        child: Row(children: [
          Expanded(flex: 3, child: Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(name, style: _t(size: 13, weight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis),
                Text('ID: #$shortId', style: _t(size: 11, color: _muted), maxLines: 1, overflow: TextOverflow.ellipsis),
              ]),
            ),
          ])),
          Expanded(flex: 2,
          ...
          Expanded(flex: 2, child: Text(waktu, style: _t(size: 13), maxLines: 1, overflow: TextOverflow.ellipsis)),
          Expanded(flex: 2,
              child: Text(_rp(total),
                  style: _t(size: 13, weight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis)),
          Expanded(flex: 2,
            child: Align(
              ...
              child: Container(
                ...
                child: Text(_sl(status),
                    style: _t(size: 11, weight: FontWeight.w700, color: dc), maxLines: 1, overflow: TextOverflow.ellipsis),
              ),
            ),
          ),
          Expanded(flex: 1,
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          ListTile(
            ...
            title: Text('Batalkan Booking',
                style: _t(size: 14, color: Colors.red)),
            onTap: () async {
              Navigator.pop(context);
              await _firestore.collection('bookings').doc(b['id'])
